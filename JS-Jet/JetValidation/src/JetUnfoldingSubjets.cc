// Rewritten JetUnfoldingSubjets.cc to include jet matching and apply matched jets to subjet analysis with TTree support
                                            ...
                                            TowerBackground* bg, float v2, float psi2)
{
  ...
  for (auto& pair : recoToTruth)
  {
    ...
    // Fill matched jet pT histograms
    hRecoJetPtMatched->Fill(recoJet->get_pt());
    
    Jet* truthJet = recoToTruth[recoJet];
    hTruthJetPtMatched->Fill(truthJet->get_pt());
    response->Fill(recoJet->get_pt(), truthJet->get_pt());
...

{
  ...
  for (auto& pair : truthToReco)
  {
    ...
    // If this truth jet has no matched reco, count as miss
    if (truthToReco.find(truthJet) == truthToReco.end()) {
      hTruthJetPtMatched->Fill(truthJet->get_pt());
      response->Miss(truthJet->get_pt());
    }
...
int JetUnfoldingSubjets::Init(PHCompositeNode*) {
  ...
  float truth_ptmax = 60;  // Based on your hTruthJetPtMatched plot (~35 GeV max)
  
  int nbins_reco = 20;
  int nbins_truth = 10;  // Fewer bins, since truth is narrow
  
  response = new RooUnfoldResponse(nbins_reco, reco_ptmin, reco_ptmax,
...
int JetUnfoldingSubjets::End(PHCompositeNode*) {
  ...
  // Perform unfolding
  RooUnfoldBayes unfold(response, hRecoJetPtMatched, 4);  // 4 iterations is typical
  unfold.SetNToys(0);  // Disable covariance calculation → prevents crash
  ...
  TH2D* hResponseMatrix = (TH2D*) response->Hresponse();
  hResponseMatrix->Write("hResponseMatrix");
...
//____________________________________________________________________________..
  ...
  {
     std::cout << "JetUnfoldingSubjets::Print(const std::string &what) const Printing info for " << what << std::endl;
  }
