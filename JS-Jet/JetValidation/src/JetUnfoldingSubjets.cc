// --------- No energy scale correction  applied
...
// -- GLOBAL pt bins --
std::vector<float> pt_bins = {10, 15, 20, 25, 30, 35};
  ...
  for (auto truthJet : *truthJets) {
    ...
    if (truthsubjets.size() >= 2 && truthsubjets[0].pt() >= 3 && truthsubjets[1].pt() >= 3) {
      ...
      if (bin >= 0) {
        ...
        m_responseZsj[bin]->Miss(truth_z_sj, truthJet->get_pt());
      }
    }
  }
}
...
int JetUnfoldingSubjets::Init(PHCompositeNode*) {
  ...
  // 1D pT response
  double reco_ptmin = 5; // --- Doubles makes the ROOUnfoldResponse object happy, floats can trigger undefined behavior 
  double reco_ptmax = 60;
  double truth_ptmin = 10;
  double truth_ptmax = 60;
  int nbins_reco = 11;
  int nbins_truth = 10;

  m_response1D = std::make_unique<RooUnfoldResponse>(
						     static_cast<int>(nbins_reco), static_cast<double>(reco_ptmin), static_cast<double>(reco_ptmax),
    static_cast<int>(nbins_truth), static_cast<double>(truth_ptmin), static_cast<double>(truth_ptmax));
  ...
  for (size_t i = 0; i < pt_bins.size() - 1; ++i) {
    ...
    std::cout << "passed 4 " << i << std::endl;
    
    TH1F* hTruth = new TH1F(("hTruthZsj_" + label).c_str(),
                            Form("Truth z_{sj} [%d-%d GeV];z_{sj};Entries", (int)ptlow, (int)pthigh),
                            20, 0, 0.5);
    std::cout << "passed 5 " << i << std::endl;

    hReco->SetDirectory(nullptr);
    hTruth->SetDirectory(nullptr);
    
    //    RooUnfoldResponse *resp = RooUnfoldResponse(hReco, hTruth);
    std::unique_ptr<RooUnfoldResponse> resp = std::make_unique<RooUnfoldResponse>(hReco, hTruth);
    resp->SetName(Form("m_responseZsj_%zu", i));
    std::cout << "passed 6 " << i << std::endl;
...
int JetUnfoldingSubjets::End(PHCompositeNode*) {
  ...
  m_response1D->Hresponse()->Write();
  for (size_t i = 0; i < m_responseZsj.size(); ++i) {
    m_responseZsj[i]->Hresponse()->Write();
  }
  
  // Unfold pT
  //RooUnfoldBayes unfold(m_response1D, hRecoJetPtMatched, 4);
  RooUnfoldBayes unfold(m_response1D.get(), hRecoJetPtMatched, 4);
  ...
  TH2* hResponseMatrix = m_response1D->Hresponse();
  if (hResponseMatrix) hResponseMatrix->Write("responsePt");
  
  // Unfold z_sj
  for (size_t i = 0; i < m_responseZsj.size(); ++i) {
    RooUnfoldBayes unfold(m_responseZsj[i].get(), m_hRecoZsjMatched[i], 4);
    unfold.SetNToys(0);
