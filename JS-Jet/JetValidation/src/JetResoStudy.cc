...
int JetResoStudy::Init(PHCompositeNode *topNode)
{
  ...
  hFakeJetPt = new TH1F("hFakeJetPt", "Fake Jet pT; p_{T} (GeV/c);Counts", 100, 0, 100);

  // Initialize TTree
  m_T = new TTree("JetTree", "Tree for matched and unmatched jets");
  m_T->Branch("jetPt", &m_jetPt, "jetPt/F");
  m_T->Branch("jetEta", &m_jetEta, "jetEta/F");
  m_T->Branch("jetPhi", &m_jetPhi, "jetPhi/F");
  m_T->Branch("deltaR", &m_deltaR, "deltaR/F");
  m_T->Branch("isMatched", &m_isMatched, "isMatched/O");
...
{
  //  std::cout << "JetResoStudy::process_event(PHCompositeNode *topNode) Processing Event" << std::endl;
  ++m_event;
  ...
  // matching or unmatching vectors ///////////////
  if (!jets || !jetsMC) {
    std::cout << "Error: Jet containers not found!" << std::endl;
    return Fun4AllReturnCodes::ABORTEVENT;
  }

  // Vectors to store matched and fake jets
  std::vector<Jet*> matchedJets;
  std::vector<Jet*> fakeJets;
  

  // looping over trruth jets ///////////////////////
  // Loop over truth jets
  for (auto truthJetIter = jetsMC->begin(); truthJetIter != jetsMC->end(); ++truthJetIter) {
    Jet* truthJet = truthJetIter->second;
    bool isMatched = false;

    // Loop over reco jets to find a match
    for (auto recoJet : *jets) {
      float deltaR = calculateDeltaR(truthJet->get_eta(), truthJet->get_phi(),
				     recoJet->get_eta(), recoJet->get_phi());
      if (deltaR < 0.4) {
	matchedJets.push_back(recoJet);
	isMatched = true;
	break;  // Assuming one-to-one matching
      }
    }

    // If no reco jet matched this truth jet, it's a fake
    if (!isMatched) {
      fakeJets.push_back(truthJet);
    }
  }
  // Now, handle the opposite: reco jets that have no matching truth jet
  for (auto recoJet : *jets) {
    bool isMatched = false;

    // Check if the reco jet is already in the matched list
    for (auto matchedJet : matchedJets) {
      if (matchedJet == recoJet) {
	isMatched = true;
	break;
      }
    }

    // If not matched, consider it a fake jet
    if (!isMatched) {
      fakeJets.push_back(recoJet);
    }
  }

  // Loop over matched jets to fill histograms and TTree
  for (auto recoJet : matchedJets) {
    m_jetPt = recoJet->get_pt();
    m_jetEta = recoJet->get_eta();
    m_jetPhi = recoJet->get_phi();
    m_isMatched = true;
    m_T->Fill();

    hRecoJetPt->Fill(m_jetPt);
  }

  // Loop over unmatched reco jets to fill histograms
  for (auto recoJet : fakeJets) {
    m_jetPt = recoJet->get_pt();
    m_jetEta = recoJet->get_eta();
    m_jetPhi = recoJet->get_phi();
    m_isMatched = false;
    ...
    hFakeJetPt->Fill(m_jetPt);
  }

  // Loop over truth jets for matched and fake
  for (auto truthJetIter = jetsMC->begin(); truthJetIter != jetsMC->end(); ++truthJetIter) {
    Jet* truthJet = truthJetIter->second;
    m_jetPt = truthJet->get_pt();
    m_jetEta = truthJet->get_eta();
    m_jetPhi = truthJet->get_phi();

    bool isMatched = false;
    for (auto recoJet : matchedJets) {
      m_deltaR = calculateDeltaR(truthJet->get_eta(), truthJet->get_phi(),
				 recoJet->get_eta(), recoJet->get_phi());
      if (m_deltaR < 0.4) {
	isMatched = true;
	break;
    ...
    if (isMatched) {
      m_isMatched = true;
      hTruthJetPt->Fill(m_jetPt);
      hDeltaR->Fill(m_deltaR);
    } else {
      m_isMatched = false;
    }
  }
